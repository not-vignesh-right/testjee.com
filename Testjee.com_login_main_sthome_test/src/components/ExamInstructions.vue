      ...
      <div class="bg-white border-t border-gray-200 pt-6">
        ...
        <div class="flex justify-between items-center">
          ...
          <div class="flex flex-col items-end gap-2">
            ...
            <button 
              @click="startExam"
              :disabled="!hasReadInstructions || isDecrementing || examStore.isLoadingQuestions"
              class="px-8 py-3 rounded-lg font-bold text-white shadow-lg transition-all flex items-center gap-2"
              :class="hasReadInstructions && !isDecrementing && !examStore.isLoadingQuestions
                ? 'bg-blue-600 hover:bg-blue-700 cursor-pointer transform hover:scale-[1.02]' 
                : 'bg-gray-400 cursor-not-allowed opacity-70'"
            >
              <svg v-if="isDecrementing || examStore.isLoadingQuestions" class="animate-spin h-4 w-4 text-white" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
              {{ examStore.isLoadingQuestions ? 'Loading questions...' : isDecrementing ? 'Starting...' : 'I am ready to begin' }}
            </button>
          </div>
        </div>
      </div>
